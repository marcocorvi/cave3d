...
class DEMgridParser extends ParserDEM
{
  ...
  boolean readData( float xwest, float xeast, float ysouth, float ynorth ) 
  {
    ...
    // Log.v("TopoGL-DEM", "bounds " + mNr1 + "x" + mNr2 + " offset " + xoff + "-" + yoff + " E " + mEast1 + " " + mEast2 + " N " + mNorth1 + " " + mNorth2 ); 
    if ( mNr1 <= 1 || mNr2 <= 1 ) {
      mValid = false;
      return false;
    }
    ...
    try {
      ...
      for ( ; k < mNr2; ++ k ) {
        line = line.trim();
        String[] vals = line.replaceAll("\\s+", " ").split(" ");
        // Log.v("TopoGL", " " + j + " " + (mNorth1+j*mDim2) + ": " + line );
        try {
          if ( flip_horz ) {
  ...
  {
    // Log.v("TopoGL-DEM", "read header " + filename );
    flip_vert = false;
    flip_horz = false;
    try {
      ...
      while ( ( line = br.readLine() ) != null ) {
        ...
        if ( line.startsWith("grid-flip ") ) {
          // Log.v("TopoGL-DEM", "grid-flip: " + line );
          String[] vals = line.replaceAll("\\s+", " ").split(" ");
          if ( vals[1].startsWith("vert") ) {  flip_vert = true; }
          else if ( vals[1].startsWith("horiz") ) { flip_horz = true; }
          else { Log.e("TopoGL-DEM", "unknown flip " + vals[1] ); }
          continue;
        }
        // try a data line:
        try {
          // Log.v("TopoGL-DEM", "try data: " + line );
          String[] vals = line.replaceAll("\\s+", " ").split(" ");
          Float.parseFloat( vals[0] );
          break;
      ...
      fr.close();
      // Log.v("TopoGL-DEM", "dim " + cols + "x" + rows + " LLcorner " + xll + " " + yll + " cell " + mDim1 + " " + mDim2 + " flip " + flip_vert + " " + flip_horz );
    } catch ( IOException e1 ) { 
      return false;
    } catch ( NumberFormatException e2 ) {
      return false;
    }
    // Log.v("TopoGL-DEM", "cell " + mDim1 + " " + mDim2 + " X " + xll + " Y " + yll + " Nx " + cols + " Ny " + rows );
    return true;
  }
