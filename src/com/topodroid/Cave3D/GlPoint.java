...
class GlPoint extends GlShape
{
  ...
  void setLocation( Vector3D w, double xmed, double ymed, double zmed )
  {
    ...
    // Log.v("TopoGL-GPS", "Med " + xmed + " " + ymed + " " + zmed );
    // Log.v("TopoGL-GPS", "geo " + w.x + " " + w.z + " " + (-w.y) );
    // Log.v("TopoGL-GPS", "loc " + mLoc[0] + " " + mLoc[1] + " " + mLoc[2] );

    mHasLoc = true;
    if ( dataBuffer == null ) { // FIXME this logic could go in GlShape as well
      // Log.v("TopoGL", "init data buffer - count " + mCount );
      initDataBuffer( mLoc );
    } else {
      // Log.v("TopoGL", "set data buffer - count " + mCount );
      dataBuffer.rewind();
      dataBuffer.put( mLoc );
    }
  }
  ...
  static void initGL( Context ctx ) 
  {
    ...
    // Log.v("TopoGL", "init GL " + mProgram + " attrs " + mAPosition + " " + mAColor + " " + mUPointSize + " " + mUMVPMatrix );
  }

  void draw( float[] mvpMatrix )
  {
    if ( mHasLoc && mCount > 0 ) {
      // Log.v("TopoGL", "draw " + mCount + " points" ); 
