...
class HullAngle
{
  ...
  void dump()
  {
    Log.v("TopoGL-ANGLE", station.short_name + " Ber " + deg(ber) + " a1 " + deg(a1) + " a2 " + deg(a2) + " Vber " + vBer.x + " " + vBer.y + " " + vBer.z );
  }
  ...
  Vector3D projectSplay( Cave3DShot splay, Vector3D v )
  {
    if ( isAlongside( v, splay.len, TopoGL.mSplayThr ) ) {
      // Log.v("TopoGL-ANGLE", "alongside V " + v.x + " " + v.y + " " + v.z + " len " + splay.len );
      return null;
    }
    Vector3D ret = null;
    if ( isRight( splay ) ) {
  ...
  void update( double b )
  {
    ...
    // Log.v("TopoGL-ANGLE", station.short_name + " update " + choice + " " + deg(ber) + " with " + deg(b) + " " + deg(a1) + " " + deg(a2) );
  }
