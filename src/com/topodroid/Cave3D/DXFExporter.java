	...
	// offset axes legends by 1
        ...
        // if ( b_legs ) 
        {
          ...
          for ( Cave3DShot blk : legs ) {
            ...
            if ( fs != null && ts != null ) {
              handle = printSegment( pw4, handle, "LEG", fs.e, fs.n, fs.z, ts.e, ts.n, ts.z );
            }
          }
        ...
        if ( b_splays ) {
          ...
          for ( Cave3DShot blk : splays ) {
            Cave3DStation fs = blk.from_station;
            Cave3DVector  ts = blk.to3DPoint();
            if ( fs != null && ts != null ) {
              handle = printSegment( pw5, handle, "SPLAY", fs.e, fs.n, fs.z, ts.x, ts.y, ts.z );
            }
          }
