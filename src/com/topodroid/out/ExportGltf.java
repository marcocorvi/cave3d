...
import com.topodroid.Cave3D.GlNames;
import com.topodroid.Cave3D.GlLines;
import com.topodroid.Cave3D.GlSurface;
...
public class ExportGltf
{
  ...
  public boolean write( String pathname )
  {
    ...
    try {
      ...
      Log.e("TopoGL", "glTF export error " + e );
      StackTraceElement[] st = e.getStackTrace();
      for ( StackTraceElement ste : st ) {
        Log.e("TopoGL", ste.toString() );
      }
    ...
    try {
      ...
      if ( names != null && names.getVertexData() != null ) {
        ...
        if ( fcount > 0 ) {
          ...
          int offset = 0;
          while ( offset < flen ) {
            // Log.v("TopoGL", "Names file " + filepath + " stride " + fstride + " coords " + fcoords + " flen " + flen + " offset " + offset );
          ...
          // Log.v("TopoGL", "Names file " + filepath + " written len " + len + " offset " + offset );
        }
      }
      dos.flush();
      dos.close();
    } catch ( FileNotFoundException e ) { 
      Log.e("TopoGL", "File " + filepath + " not found");
    } catch ( IOException e ) {
      Log.e("TopoGL", "File " + filepath + " write failed");
    }
    return len;
  }
    ...
    try {
      ...
      if ( lines != null && lines.getVertexData() != null ) {
        ...
        if ( fcount > 0 ) {
          ...
          int offset = 0;
          // Log.v("TopoGL", "Lines file stride " + fstride + " coords " + fcoords + " flen " + flen + " offset " + offset + " count " + fcount );
          while ( offset < flen ) {
          ...
          // Log.v("TopoGL", "Lines file written len " + len + " end offset " + offset );
        }
      }
      dos.flush();
      dos.close();
    } catch ( FileNotFoundException e ) { 
      Log.e("TopoGL", "File LINES not found");
    } catch ( IOException e ) {
      Log.e("TopoGL", "File LINES write failed");
    }
    return len;
  }
  ...
  private int doExportTriangles( String filepath, GlSurface surface, MinMax minMaxP, MinMax minMaxN )
  {
    ...
    try {
      ...
      Log.e("TopoGL", "File SURFACE not found");
    } catch ( IOException e ) {
      Log.e("TopoGL", "File SURFACE write failed");
    }
    return len;
  }
  ...
  private int getNextChunk( byte[] buf, int clen, float[] array, int flen, int offset, int len, int stride, MinMax minMax )
  {
    ...
    int bpos = 0;
    int klen = clen / (4*len);
    // Log.v("TopoGL", "Get chunk Flen " + flen + " offset " + offset + " len " + len + " klen " + klen );
    // sb.append("[" + klen + "]" );
    ...
    // Log.v("TopoGL", "Chunk " + sb.toString() );
    return bpos;
  }
