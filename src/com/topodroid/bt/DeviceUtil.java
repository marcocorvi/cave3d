...
public class DeviceUtil
{
  ...
  //   Log.v("Cave3D", "PAIRING: " + device.getName() + " " + device.getAddress() );
  //   try { 
  //     device.getClass().getMethod("setPairingConfirmation", boolean.class).invoke(device, true);
  //     Log.v("Cave3D", "done setPairingConfirmation");
  ...
  //     Log.v("Cave3D", "done setPin");
  //     Class[] classes3 = new Class[ 0 ];
  //     Method m_createBond = device.getClass().getMethod( "createBond", classes3 );
  //     m_createBond.invoke( device );
  //     Log.v("Cave3D", "done createBond");
  //   } catch ( NoSuchMethodException e ) {
  //     Log.v("Cave3D", "No Such method: " + e.getMessage() );
  //   } catch ( IllegalAccessException e ) {
  //     Log.v("Cave3D", "Illegal access: " + e.getMessage() );
  //   } catch ( InvocationTargetException e ) {
  //     Log.v("Cave3D", "Invocation target: " + e.getMessage() );
  //   }
  // }

  public static void bindDevice( BluetoothDevice device, String pin_code )
  {
    Log.v( "Cave3D", " bind device ... " + ((device==null)? "null" : device.getAddress()) );
    ...
    try {
      ...
      Log.v( "Cave3D", "Failed to set PIN: no method " + e.getMessage() );
    } catch ( InvocationTargetException e ) {
      Log.v( "Cave3D", "Failed to set PIN: invoke " + e.getMessage() );
    } catch (IllegalAccessException e ) {
      Log.v( "Cave3D", "Failed to set PIN: illegal access " + e.getMessage() );
    }
  }
...
/*
  public static void checkPairing( String address )
  {
    if ( TDSetting.mAutoPair ) { // try to get the system ask for the PIN
      Log.v( "Cave3D", " check pairing " + address );
      BluetoothDevice btDevice = getRemoteDevice( address );
      // Log.v( "Cave3D", "auto-pairing remote device " + btDevice.getAddress() + " status " + btDevice.getBondState() );
      if ( ! isPaired( btDevice ) ) {
        pairDevice( btDevice );
        bindDevice( btDevice );
        for (int c=0; c<TDSetting.mConnectSocketDelay; ++c ) {
          if ( isPaired( btDevice ) ) break;
          DeviceType.slowDown( 100 ); // Thread.yield();
        }
      }
    }
  }
