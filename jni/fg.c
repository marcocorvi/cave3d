/* @file fg.c
 *
 * @brief  face graph of hull, and splay trees 
 *
 * --------------------------------------------------------
...
void visit_fg_i(   void (*v_fg)(Tree *, int, int),
    ...
    if (t->fgs->mark!=vn) {
        t->fgs->mark = vn;
        if ( isAtInfinity( t->key ) // t->key!=coordsAtInfinity
             && !mo[site_num(t->key)] ) boundaryc = 0;
...
int visit_fg_i_far(void (*v_fg)(Tree *, int),
    ...
    if (t->fgs->mark!=vn) {
        t->fgs->mark = vn;
        nb = isAtInfinity( t->key ) // (t->key==coordsAtInfinity)
             || mo[site_num(t->key)];
...
void p_fg(Tree* t, int depth, int bad) {
    ...
    if (mults[0]==0) {
        mults[pdim] = 1;
        for (i=pdim-1; i>=0; i--) mults[i] = toScaled( mults[i+1] );
    }
...
#if 0
double fg_hist[100][100];
double fg_hist_bad[100][100];
double fg_hist_far[100][100];
...
// // declaration from ch.c
// double find_alpha(simplex *root, double * mins, double * maxs );
// 
// // unused
// void print_hist_fg(simplex *root, fg *faces_gr, FILE *F, double * mins, double * maxs ) {
//     int i,j,k;
